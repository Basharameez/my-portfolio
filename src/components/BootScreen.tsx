...
export const BootScreen = ({ onComplete }: BootScreenProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#FDFCFA] z-50 flex items-center justify-center p-4 font-mono select-none">
      <div className="w-full max-w-xl border-4 border-double border-[#1A1A24] bg-[#F4F4F4] p-6 shadow-[8px_8px_0px_#FF3D00] relative overflow-hidden">
        {/* Terminal Header */}
        <div className="flex items-center justify-between border-b-2 border-[#1A1A24] pb-3 mb-4">
        ...
        {/* Console Log Stream */}
        <div className="h-64 overflow-y-auto flex flex-col gap-1.5 text-xs text-[#1A1A24] select-text font-bold">
          {logs.map((log, i) => (
